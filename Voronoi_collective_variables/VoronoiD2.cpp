...
public:
  explicit VoronoiD2(const ActionOptions&);
  ~VoronoiD2();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiD2,"VORONOID2")

void VoronoiD2::registerKeywords( Keywords& keys ) {
...
VoronoiD2::VoronoiD2(const ActionOptions&ao):
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0;j<num_atomsa;j++) {
      for(unsigned int k=j+1;k<num_atomsa;k++) {
		...
		//only consider distance between [O in water] and [N,O,O in glycine], [N in glycine] and [O,O in glycine]
		if (j<num_atomso+1 && k>num_atomso-1) {
        ...
        IonDistance -= buffer; //this is the IonDistance for Glycine, the result!
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
        ...
        for(unsigned int k=0;k<num_atomsa;k++) {
          ...
          for(unsigned int j=k+1;j<num_atomsa;j++) {
			...
			if (k<num_atomso+1 && j>num_atomso-1) {
