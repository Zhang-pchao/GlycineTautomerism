...
class VoronoiS1 : public Colvar {
  ...
  double d0, d1, r0, sum_exp;

public:
  explicit VoronoiS1(const ActionOptions&);
  ~VoronoiS1();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiS1,"VORONOIS1")

void VoronoiS1::registerKeywords( Keywords& keys ) {
...
VoronoiS1::VoronoiS1(const ActionOptions&ao):
...
// calculator
void VoronoiS1::calculate()
{

  double solvation=0.0;
  vector<double> dfunc_theta(2); //devirative factor for solvation
  fill(dfunc_theta.begin(),dfunc_theta.end(),0.);
  vector<double> c_tot(2); // c_tot[0]: charge of all O in water //c_tot[1]: charge of N O O in glycine
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    //for(unsigned i=num_atomso; i<num_atomsa; i++) charge[i]+=charget[i];  
    #pragma omp barrier

    //LOOP2.5 shift charge by d0
    #pragma omp for reduction(+:solvation)
    //for(unsigned int j=num_atomso; j<num_atomsa; j+=1) {
    for(unsigned int j=0; j<num_atomsa; j+=1) {		
      ...
      if(j<num_atomso) c_tot[0]+=charge[j];    //charge of all O in water
      else c_tot[1]+=charge[j];                //charge of N O O in glycine            
    }
	for(unsigned int i=0;i<2;i++) {  //WARNING it works only for 2 species, not more and not less!
		solvation += pow(2,i)*(c_tot[i]-d[i]);
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
		...
		if(ind0 < num_atomso) {
			buf=fk*dfunc_theta[0];
		} else {
			buf=fk*dfunc_theta[1];
		}	
