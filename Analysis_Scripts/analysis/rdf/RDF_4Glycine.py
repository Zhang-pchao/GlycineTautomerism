...
def get_rdf(c1,c2,u,NOOdict,type_map,trj_skip,mda_step,save_path):
    s1_ = 'index'
    for i in c1:
        s1_ += ' %d'%(NOOdict[i]-1)
    ...
    file = open(os.path.join(save_path, "RDF_{0}*_{1}.txt".format(c1[0][0],c2)), 'w+')
    file.write("#RDF %4s*-%s\n"%(c1[0][0],c2))
    file.write('#%8s%8s\n' %('bin','rdf'))
...
x_rdf1,y_rdf1 = get_rdf(['N'],'O',u,NOOdict,type_map,trj_skip,mda_step,save_path)
get_fig('N*','O',x_rdf1,y_rdf1,colors,save_path)
x_rdf2,y_rdf2 = get_rdf(['N'],'H',u,NOOdict,type_map,trj_skip,mda_step,save_path)
get_fig('N*','H',x_rdf2,y_rdf2,colors,save_path)
x_rdf3,y_rdf3 = get_rdf(['O1','O2'],'O',u,NOOdict,type_map,trj_skip,mda_step,save_path)
get_fig('O*','O',x_rdf3,y_rdf3,colors,save_path)
x_rdf4,y_rdf4 = get_rdf(['O1','O2'],'H',u,NOOdict,type_map,trj_skip,mda_step,save_path)
get_fig('O*','H',x_rdf4,y_rdf4,colors,save_path)

get_allfig(['N*','N*','O*','O*'],['O','H','O','H'],
