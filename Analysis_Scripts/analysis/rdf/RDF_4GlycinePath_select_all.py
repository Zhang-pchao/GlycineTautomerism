...
def get_rdf(c1,c2,u,NOOdict,type_map,trj_skip,mda_step,save_path,save_name):
    ...
    mkdir(final_save)
    file = open(os.path.join(final_save, "RDF_{0}_{1}g_{2}a.txt".format(save_name,c1[0][0],c2)), 'w+')
    file.write("#RDF %4s*-%s\n"%(c1[0][0],c2))
    file.write('#%8s%8s\n' %('bin','rdf'))
...
def get_allfig(eles,path_idx,x_rdf,y_rdf,colors,save_path):
    ...
    ax.set_xlabel("r "+r"$\ \rm (\AA)$")
    ax.set_ylabel("g(r) of [$\mathregular{%s^G}$ – $\mathregular{%s^A}$]"%(eles[0],eles[1]))
    
    fig.savefig(os.path.join(save_path, "RDF_%sg_%sa_all.png"%(eles[0],eles[1])), dpi=600, bbox_inches='tight')
...
for j in range(len(eles)):
    ...
    for i in range(len(trjs)): 
        ...
        x,y = get_rdf(ele_list,'%s'%eles[j][1],u,NOOdict,type_map,trj_skip,mda_step,save_path,save_name)
