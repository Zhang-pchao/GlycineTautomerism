...
parser.add_argument('--penergy',dest='penergy',type=str,help='the potential energy to be used')
# grid related
parser.add_argument('--min',dest='grid_min',type=str,default="-1.2,-0.5",help='lower bounds for the grid')
parser.add_argument('--max',dest='grid_max',type=str,default="1.2,12.5", help='upper bounds for the grid')
...
parser.add_argument('--skipfoot',dest='skipfoot',type=int,default=0,help='skip this number of bottom  rows')
parser.add_argument('--totalrow',dest='totalrow',type=int,default=0,help='total rows')
...
parser.add_argument('--nomintozero',dest='nomintozero',action='store_true',default=True,help='do not shift the minimum to zero')
...
args_cv=args.cv
if args.penergy is not None: #U is used for TS=H-A=U+PV-A=U-A
    args_ene=args.penergy
args_bias=args.bias
...
args_skipfoot=args.skipfoot
args_totalrow=args.totalrow
...
if args.penergy is not None: #U is used for TS=H-A=U+PV-A=U-A
    col_ene=[]
    try:
        col_ene=int(args_ene.split(',')[0])-1
        name_ene=fields[col_ene+2]
    except ValueError:
        col_ene_flag=-1
        name_ene=args_ene.split(',')[0]
        for i in range(len(fields)):
            if fields[i]==name_ene:
                col_ene.append(i-2)
                col_ene_flag=-2
        if col_ene_flag==-1:
            sys.exit(error%('energy "%s" not found'%name_ene))    
# get bias
...
for enes in col_ene:  
  print(' using energy "%s" found at column %d'%(name_ene,enes+1))
# get periodicity
period_x=0
period_y=0
header_lines=1
...
# read file
all_cols=[col_x]+col_bias+col_ene
if dim2:
  all_cols=[col_x,col_y]+col_bias+col_ene
all_cols.sort() #pandas iloc reads them ordered
data=pd.read_table(filename,dtype=float,sep='\s+',comment='#',header=None,usecols=all_cols,skiprows=skipme,skipfooter=args_skipfoot,engine="python")
...
pot_ene=np.array(data.iloc[:,all_cols.index(col_ene[0])])
pot_ene=pot_ene-np.amin(pot_ene) # for safe exp calcution below 
del data
size=0
effsize=0
...
#actual printing
  ...
  f.write('#! SET total_sample_size %d\n'%(args_totalrow-args_skipfoot))  
...
def calcFESpoint(start,end,point_x,point_y=None): # potential energy point  
  enetmp=[]
  cutoff_x=grid_cv_x[1]-grid_cv_x[0]
  if point_y is not None: 
    cutoff_y=grid_cv_y[1]-grid_cv_y[0]
    for i in range(len(cv_x[start:end])):
        if point_x-cutoff_x<cv_x[start+i]<point_x+cutoff_x and point_y-cutoff_y<cv_y[start+i]<point_y+cutoff_y:
            enetmp.append(pot_ene[start+i])
  else:
    for i in range(len(cv_x[start:end])):
        if point_x-cutoff_x<cv_x[start+i]<point_x+cutoff_x:
            enetmp.append(pot_ene[start+i])  
  if enetmp==[] or len(enetmp)<10:
    return -1.0 
  else:
    return np.average(enetmp) # min energy=0 
# adjust stride
